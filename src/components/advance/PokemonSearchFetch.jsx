...
const typeColors = {
  fire: "bg-red-500", water: "bg-blue-500", grass: "bg-green-500", electric: "bg-yellow-500", psychic: "bg-pink-500",
  ice: "bg-blue-200", dragon: "bg-purple-500", dark: "bg-gray-800", fairy: "bg-pink-300", normal: "bg-gray-400",
  fighting: "bg-orange-700", flying: "bg-indigo-300", poison: "bg-purple-700", ground: "bg-yellow-700",
  rock: "bg-yellow-900", bug: "bg-green-700", ghost: "bg-purple-900", steel: "bg-gray-500",
};

const PokemonSearchFetch = () => {
  const [pokemon, setPokemon] = useState(null), [query, setQuery] = useState(""), [error, setError] = useState(null), [suggestions, setSuggestions] = useState([]), [isFocused, setIsFocused] = useState(false);
      ...
      <form onSubmit={searchPokemon} className="w-full max-w-sm relative">
        ...
        {isFocused && suggestions.length > 0 && (
          <ul className="absolute top-12 left-0 w-full bg-white border rounded shadow-md max-h-60 overflow-y-auto z-10">
            {suggestions.filter((name) => name.toLowerCase().startsWith(query.toLowerCase())).map((name) => (
              <li
                key={name}
        ...
        <div className="mt-6 p-4 bg-white rounded shadow-md w-full max-w-md">
          ...
          <div className="mt-4 flex justify-center">
            {pokemon.types.map((typeInfo) => (
              <span key={typeInfo.type.name} className={`px-4 py-1 rounded-full text-white ${typeColors[typeInfo.type.name]} mx-1`}>
                {typeInfo.type.name}
              </span>
            ))}
          </div>
